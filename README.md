...
6. tableタグの `height="100%"`の削除
![sie_sample](image/sie_sample.png)

- 青枠の部分が高さの解釈で変なので、`height="100%"`を削除する

- 対処方法：正規表現`\s+height=["']100%["']`でヒットするものをgrep置換する

## 適用方法（概要）

対象環境の元ファイルをバックアップしたうえで、上記6項目を対応箇所へ反映してください。
